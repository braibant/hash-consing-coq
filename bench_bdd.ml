...
let test name formula =     
  Printf.printf "%s\tFormula:%.12s\t%s\n"  (String.make 6 '=') name (String.make 6 '=');
  (* reference implementation *)
  (* Gc.major (); Printf.printf "Live words:%L\n" ((Gc.stat ()).Gc.live_words); *)
  (* let f,tref    = time Reference.mk formula in  *)
  (* let rref,tref'   = time Reference.is_tauto f in  *)
  (* Printf.printf "reference bdd:%f check:%f\n%!" tref   tref'; *)
  
  (* Pure implementation *)  
  Gc.major (); Printf.printf "Live words:%L\n" ((Gc.stat ()).Gc.live_words);
  let f,tpure   = time Pure.mk formula in
  let fpure, bdd = match f with | Some (f,bdd) -> f, bdd | None -> assert false in 
  let rpure,tpure'  = time Pure.is_tauto fpure in 
  Printf.printf "pure      bdd:%f check:%f\n%!" tpure  tpure';

  (* Smart implementation *)    
  Gc.major (); Printf.printf "Live words:%L\n" ((Gc.stat ()).Gc.live_words);
  let fsmart,tsmart  = time Smart.mk formula in 
  let rsmart,tsmart' = time Smart.is_tauto fsmart in 
  Printf.printf "smart     bdd:%f check:%f\n%!" tsmart tsmart';
  Printf.printf "results:  %b %b \n%!" rpure rsmart
  
let _ = 
  for i = 1 to 6 do
    begin 
      let i = i * 250 in 
      let formula,t1 = time Urquahrt.main i in 
      let name = Printf.sprintf "Urquhart-%.3i (%fs,%i) " i t1 (size formula) in 
	test name formula
    end;
    begin
      let i = i * 25 in 
      let formula,t1 = time Pigeon.main i in 
      let name = Printf.sprintf "Pigeonhole-%.3i (%fs,%i)" i t1 (size formula) in
	test name formula
    end
  done
;;
...
let _ = 
  ...
  begin
    let formula,t1 = time Pigeon.main i in 
    let name = Printf.sprintf "Pigeonhole-%.3i (%fs)" i t1 in
    test name formula
  end
;;
*)
